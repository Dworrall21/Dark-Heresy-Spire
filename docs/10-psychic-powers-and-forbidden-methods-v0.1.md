# Psychic Powers and Forbidden Methods v0.1

Psychic Powers and Forbidden Methods are not balanced because they are weak.

They are balanced because they work.

Acolytes should be tempted to use them when ordinary investigation, authority, violence, or requisition is too slow. The cost is not that the method fails. The cost is that it leaves marks: on the user, on the victim, on the Operation, on the Patron’s patience, and on the soul of the cell.

This document expands the core examples from Core Rules v0.2 into table-facing procedures, tags, burdens, power chassis, forbidden method chassis, and generators.

---

## 1. Core Principle

When a Psychic Power or Forbidden Method is used, ask:

1. What does this solve?
2. Why is it faster than the orthodox approach?
3. What does it reveal?
4. Who could notice?
5. What trace does it leave?
6. What Resistance is risked?
7. What does the Patron think this means?
8. What enemy, faction, daemon, machine-spirit, witness, or rival now has leverage?

Do not make these options unattractive by making them unreliable.

Make them attractive, effective, and costly.

---

## 2. Psychic Powers vs. Forbidden Methods

| Type | What It Is | Primary Risk |
|---|---|---|
| **Psychic Power** | A direct act of sanctioned or unsanctioned psychic force. | Corruption, Mind, Warp Trace, Heat, Enemy Awareness |
| **Forbidden Method** | A procedure, tool, bargain, atrocity, legal abuse, or radical shortcut. | Authority, Shadow, Corruption, Patron Notice, Red Lines |
| **Forbidden Asset** | A person, device, relic, daemonhost, xenos tool, or heretek system used as a Resource. | Burden Profiles, faction claims, dependency, exposure |
| **Radical Doctrine** | A repeated pattern of using forbidden methods as policy. | Patron judgment, rival Inquisition, permanent consequences |

Psychic Powers are usually moment-to-moment actions.

Forbidden Methods are often choices, procedures, assets, or plans.

---

## 3. Psychic/Forbidden Methods and the Core Rules

Psychic Powers and Forbidden Methods use the same core machinery as the rest of **Dark Heresy Spire**. They are not separate minigames; they are high-leverage ways to change what is possible and what the table must pay for afterward.

| Element | Core Rule Interaction |
|---|---|
| **Permission** | A power, rite, asset, or radical shortcut lets the cell attempt something ordinary methods cannot. |
| **Mastery** | A prepared focus, discipline, forbidden asset, or known method may grant +1D10. Mastery does not stack. |
| **Difficulty** | Wards, exposure, unstable Warp conditions, witnesses, opposition, null presence, or impossible clean success remove dice. |
| **Stress Dice** | Personal cost usually lands as Corruption, Mind, Shadow, Authority, or Body stress. |
| **Protection** | Wards, sanctified tools, null fields, restraints, cover, or sanctioned paperwork may absorb specific stress when their effect says so. |
| **Resource Dice** | Forbidden Assets, psy-focuses, grimoires, daemonhosts, xenos tools, contacts, and cover channels can step down when strained. |
| **Tags** | Tags tell the GM what pressure to use: Heat, Enemy Awareness, Warp Trace, Patron Notice, Red Line pressure, or Burdens. |
| **Heat** | Public noise, witnesses, recordings, visible miracles, purges, and official panic make the Operation hotter. |
| **Enemy Awareness** | Opposition learns a specific method, signature, asset, weakness, Patron link, or psychic fingerprint. |
| **Warp Trace** | Occult residue, dream echo, possession vector, soul impression, or psychic evidence remains after the act. |
| **Clocks and Burdens** | Delayed consequences track hunger, exposure, audits, possession, dependency, faction response, or rival attention. |
| **Patron Notice, Patience, and Red Lines** | Institutional judgment follows exposed witchcraft, Radical practice, hidden assets, disobedience, and crossed constraints. |

### Calibration Guidance

Choose the pressure that says what actually went wrong.

| Pressure | Use For |
|---|---|
| **Difficulty** | The action is riskier before the roll: hostile wards, active resistance, unstable Warp site, public scrutiny, weak cover, null interference, or a result that cannot be clean. |
| **Stress** | The acolyte personally pays: corruption, guilt, fear, backlash, exhaustion, legal strain, or damaged cover. |
| **Heat** | Outsiders notice noise, spectacle, violence, missing people, official movement, public miracle, or panic. |
| **Enemy Awareness** | The enemy learns the cell's method, signature, tool, route, Patron habit, psychic fingerprint, or forbidden dependency. |
| **Warp Trace** | The act leaves occult residue that psykers, daemons, witches, wards, machine-spirits, or victims can later read. |
| **Patron Notice / Patience / Red Lines** | The Patron or rival Inquisition has reason to judge, correct, audit, disavow, or punish the cell. |
| **Burdens or clocks** | The cost should mature later: dependency, possession, audit, faction retaliation, debt, witness panic, or contaminated clue. |

Avoid stacking every pressure on one use. Most psychic or forbidden actions should have **one base cost** plus **one exposure consequence** if the use is visible, public, compromised, or especially extreme.

### Default Difficulty by Psychic/Forbidden Approach

| Approach | Use When |
|---|---|
| **Safe** | Prepared, sanctioned, warded, private, controlled, well-covered, or strongly supported. |
| **Risky** | Rushed, observed, resisted, emotionally unstable, weakly covered, or using a strained asset. |
| **Dangerous** | Hostile wards, occult opposition, public miracle, unstable Warp site, forbidden asset under scrutiny, or Untouchable/null interference. |
| **Impossible** | Success cannot be clean without sacrifice, escalation, exposure, permanent consequence, or accepting a forbidden price. |

Impossible does not mean the method cannot work. It means it cannot work cleanly.

---

## 4. Using Psychic Powers

When a psyker uses a power under pressure, roll the listed Skill + Domain.

Most Psychic Powers use **Warp**.

Some may use:

| Domain | Use When |
|---|---|
| **Warp** | Raw psychic power, daemons, sorcery, immaterial traces, possession |
| **Inquisition** | Sanctioned protocols, interrogator rites, controlled psyker procedures |
| **Heresy** | Cult witchcraft, forbidden psychic doctrine, unsanctioned rites |
| **Ecclesiarchy** | Faith-mediated rites, public miracles, exorcistic theater |
| **Mechanicus** | Psy-tech, limiters, noospheric anomalies, machine-ward interaction |
| **Xenos** | Alien psychic logic, soul-tech, non-human mental structures |

### Psychic Power Procedure

| Step | Table Call |
|---|---|
| **1. Name the effect** | Say exactly what the power solves right now. |
| **2. Choose the roll** | Use the power's Skill + Domain, or choose the closest Skill + Domain if improvised. |
| **3. Set Difficulty** | Apply Safe, Risky, Dangerous, or Impossible from the approach table. |
| **4. Confirm cost** | Use the listed cost, default Corruption stress on 7 or lower, or a sharper cost for Unbound/Forbidden use. |
| **5. Decide visibility** | If the power is visible, loud, miraculous, or terrifying, decide Heat, witness, Cover, or Patron pressure. |
| **6. Decide trace** | Choose whether it leaves Warp Trace now or only if a cost, 7 or lower, or Fallout demands it. |
| **7. Keep the hook** | If stress becomes Fallout, use the power's Fallout Hook or a Psychic Burden. |

### Default Psychic Cost

On a **7 or lower**, the psyker takes **Corruption stress** in addition to any other consequence unless the power says otherwise.

On a **1**, the GM may immediately introduce one:

- Psychic phenomenon.
- Daemonic attention.
- Possession pressure.
- Collateral harm.
- Warp Trace.
- Heat +1.
- Enemy Awareness +1.
- A vulnerable witness becomes marked.
- A ward, relic, machine-spirit, astropath, daemon, or witch-finder notices.

### Visible Psychic Power

Visible psychic power is about **public exposure**. Warp Trace is about **occult detectability**. A power can be invisible to civilians and still leave Warp Trace, or spectacular in public without giving the enemy a useful psychic signature.

If the power is visible, loud, miraculous, terrifying, or physically impossible, choose one:

- Heat +1.
- Witnesses become unreliable but intense.
- Local faith interprets the event as miracle or witchcraft.
- Psyker's Cover is Questioned.
- Patron Notice if the cell was meant to remain covert.

**Visible powers and Cover:** Using a Visible or Area power where a Cover identity
is active creates a tension between power and secrecy. The general rule is that
Cover is stepped down by one die size (once per Situation). The Sanctioned Psyker
class has an advance — Disciplined Casting — that lets the player choose to step
down Cover *instead of* taking Heat and witness consequences, turning a penalty
into a tactical choice. See Sanctioned Psyker class.

**Psyker and Blank interaction:** The Blank's Absence ability steps up Difficulty
for psychic powers. Edge cases (Mastery, Corruption Protection, Cut the Thread,
Soulless Collapse) are defined in the Blank class and the Sanctioned Psyker's
Null Adaptation advance. See Blank class and Sanctioned Psyker class.

### Tag Pressure

| Tag | Calibration Note |
|---|---|
| **Subtle** | Do not add Heat from spectacle alone, but Warp Trace may still exist. |
| **Visible** | Heat, witnesses, Cover pressure, faith panic, or Patron Notice is likely if the scene is populated or monitored. |
| **Invasive** | Mind stress, Corruption stress, witness trauma, clue contamination, or Patron judgment is likely. |
| **Warded** | Reduce Difficulty, reduce stress, or grant Protection when preparation and proper materials matter. |
| **Sanctioned** | Reduce Authority risk inside mandate; it does not remove fear, Heat, Warp Trace, or enemy recognition. |
| **Unbound** | Increase effect, but add Corruption, Perils, Burden, or clock pressure. |
| **Possession Risk** | On a 1, Severe Corruption Fallout, or reckless repeated use, create possession pressure immediately. |

---

## 5. Psychic Power Structure

A Psychic Power is written as:

```text
Name:
Discipline:
Roll:
Effect:
Tags:
Cost:
Tell:
Trace:
Fallout Hook:
```

| Field | Meaning |
|---|---|
| **Name** | What the power is called at the table. |
| **Discipline** | Broad psychic mode: sight, compulsion, force, flame, flesh, veil, message, ward. |
| **Roll** | Skill + Domain. |
| **Effect** | What the power accomplishes on success. |
| **Tags** | Table-facing traits that affect fiction and consequences. |
| **Cost** | Stress, Heat, Enemy Awareness, or step-down. |
| **Tell** | What witnesses notice. |
| **Trace** | What remains afterward. |
| **Fallout Hook** | What goes wrong when stress becomes consequence. |

---

## 6. Psychic Tags

| Tag | Effect |
|---|---|
| **Subtle** | Does not automatically create visible phenomena. Still may leave Warp Trace. |
| **Visible** | Witnesses can see or feel the power. Heat may rise if not contained. |
| **Invasive** | Affects mind, memory, soul, dream, or identity. Often risks Mind or Corruption stress. |
| **Violent** | Inflicts Body stress or destroys objects. Often creates evidence. |
| **Area** | Affects a zone, mob, group, chamber, dream-cluster, or machine-bank. |
| **Sustained** | Continues for a scene but creates ongoing risk or trace. |
| **Warded** | Safer when prepared with rites, brands, limiters, hexagrammic chains, or sanctioned tools. |
| **Unbound** | Stronger or broader, but always risks Corruption or Perils. |
| **Miraculous** | Can inspire, terrify, or convince faithful witnesses; also creates rumor. |
| **Predatory** | The power reaches, hooks, feeds, marks, or follows a target. |
| **Echoing** | Leaves a psychic residue detectable later. |
| **Possession Risk** | On 1 or Severe Corruption Fallout, possession pressure enters play. |
| **Sanctioned** | Legally permitted under narrow conditions. Reduces Authority risk, not fear. |
| **Forbidden** | Evidence of use can create Authority stress, Patron Notice, or Puritan hostility. |

---

## 7. Warp Trace

Psychic Powers often leave traces even when nobody sees them.

| Trace | Who Can Notice | Effect |
|---|---|---|
| **Dream Echo** | Psykers, astropaths, daemons, victims | The event appears in dreams, prophecy, or nightmares. |
| **Soul Impression** | Witches, exorcists, daemonhosts, warp entities | The psyker’s emotional or spiritual fingerprint remains. |
| **Memory Scar** | Victims, telepaths, interrogators | The target remembers the power as trauma, miracle, or violation. |
| **Machine Disruption** | Tech-priests, cogitators, auspex systems | Logs glitch, picts distort, servitors pause, machine-spirits complain. |
| **Warp Scent** | Daemons, sorcerers, sanctioned psykers | Something immaterial can track the act. |
| **Miracle Rumor** | Priests, pilgrims, civilians | The social story grows faster than the facts. |
| **Possession Vector** | Daemons, cults, corrupted witnesses | A vulnerable person becomes a door. |

When Warp Trace matters, choose one:

- Heat +1 if the trace can spread publicly.
- Enemy Awareness +1 if the opposition can interpret it.
- D3 Corruption stress.
- D3 Mind stress.
- Create or tick a Warp Trace clock.
- Mark a witness, victim, or location.

### Warp Trace Clock

The Warp Trace clock is a shared 6-segment clock that tracks how much occult residue the cell has left in the current Operation area. It advances when psychic powers leave detectable traces, when daemonic phenomena occur, or when the GM determines the Warp has been disturbed.

**When to tick:** +1 segment each time a psychic power with Echoing, Visible, or Area tags is used; each time a power rolls 7 or lower; each time a daemonic entity manifests or is banished; each time a Warp breach opens or is sealed.

**When the clock fills (6/6):** The accumulated trace becomes a beacon. Choose one or more:

- **Daemon Tracking:** A daemonic entity can now locate the cell's position within the Operation area without needing line of sight or investigation. It knows where they are.
- **Warp Rift:** A thin place opens at the site of the most recent psychic use. Minor warp phenomena begin: whispers, shadows moving wrong, electronics failing, dreams bleeding into waking. The rift persists until sanctified.
- **Enemy Occult Detection:** Any enemy with psykers, astropaths, or daemonological knowledge can read the trace and learn the cell's methods, signatures, and approximate numbers. Enemy Awareness +2.
- **Patron Notice:** If the Patron has reason to monitor the cell's psychic activity, the beacon is visible to their agents. Patron Notice +1.
- **Corruption Surge:** The accumulated trace presses on everyone present. Each character in the area takes D3 Corruption stress.

The clock can be reduced by sanctification (Ecclesiarchy rites, hexagrammic wards, Mechanicus purification) at a rate of –1 segment per full Operation spent on cleansing, or –2 if a sanctioned psyker or exorcist leads the rite.

---

## 8. Psychic Power Chassis

Use a Chassis when building a new power.

| Chassis | Default Roll | Effect | Default Tags | Default Cost |
|---|---|---|---|---|
| **Witch-Sight** | Investigate + Warp | Perceive immaterial truth, traces, possession, or hidden psychic pressure. | Subtle, Echoing | D3 Mind/Corruption on 7 or lower |
| **Soul Pressure** | Command + Warp | Compel attention, freeze a target, force a short answer, or reveal emotion. | Invasive, Predatory | D6 Mind or Corruption |
| **Telekinetic Force** | Fight or Move + Warp | Push, crush, pin, hurl, disarm, shield, or breach. | Violent, Visible | D3 Body and D3 Corruption |
| **Warp Fire** | Fight + Warp | Burn, cleanse, deny space, destroy swarms, ignite ritual matter. | Violent, Area, Visible | Always D3 Corruption |
| **Mind-Scrape** | Investigate + Warp | Tear a memory, image, route, phrase, face, or passcode from a mind. | Invasive, Forbidden | D3 Mind; D3 Corruption on 7 or lower |
| **Veil** | Skulk + Warp | Blur attention, soften memory, conceal presence, misdirect witnesses. | Subtle, Sustained | D6 Shadow or Corruption |
| **Astropathic Needle** | Command or Operate + Warp | Send or receive short message through hostile psychic conditions. | Echoing, Invasive | Always D3 Mind |
| **Biomantic Surge** | Resist or Fight + Warp | Heal, harm, strengthen, poison, mutate, or force the body. | Invasive, Visible | D3 Body or Corruption |
| **Ward-Rite** | Resist + Warp or Ecclesiarchy | Resist, bind, delay, repel, or reveal daemonic/psychic force. | Warded, Holy | Stress if rushed or broken |
| **Machine Omen** | Operate + Warp/Mechanicus | Read psychic disturbance through machinery or machine-spirit behavior. | Echoing, Heretek Risk | D3 Corruption or Authority |

---

## 9. Psychic Power Modules

Add 0–2 modules to customize a power.

| D20 | Module | Adds Tags | Effect |
|---:|---|---|---|
| 1 | **Subtle Working** | Subtle | No Heat from obvious phenomena unless witnessed by psykers or sensors. |
| 2 | **Ritual Anchor** | Warded | Reduce Corruption stress by one die size if prepared. |
| 3 | **Blood Price** | Hungry | Gain Mastery, then take D3 Body or Mind stress. |
| 4 | **Hexagrammic Focus** | Warded, Sanctioned | Gain Protection 1 against psychic backlash. |
| 5 | **Overchannel** | Unbound, Unstable | Step effect up once; on 7 or lower, add D6 Corruption. |
| 6 | **Area Bloom** | Area, Visible | Affect a group or zone; Heat +1 if witnessed. |
| 7 | **Needle Precision** | Subtle, Invasive | Affect one target cleanly; on 10 leave no obvious trace. |
| 8 | **Lingering Mark** | Echoing, Predatory | Target can be tracked psychically later. They may also track you. |
| 9 | **Miracle Mask** | Miraculous | Witnesses interpret the power through faith, not sorcery. |
| 10 | **Daemon-Bait Resonance** | Forbidden, Possession Risk | Gain Mastery against daemonic phenomena; something notices. |
| 11 | **Sanctioned Protocol** | Sanctioned | Reduce Authority risk if used within mandate. |
| 12 | **Pain Cage** | Invasive, Sustained | Hold a target longer, but take D3 Mind stress each exchange. |
| 13 | **Soul-Lens** | Echoing | Ask one extra question on 10; leave a Soul Impression. |
| 14 | **Corpse Medium** | Forbidden | Use the dead as conduit; mark Corruption or Heat if discovered. |
| 15 | **Astropathic Relay** | Echoing, Area | Message reaches farther; more minds hear fragments. |
| 16 | **Psy-Limiter Break** | Unbound, Obvious | Ignore one psychic restraint; device or body steps down. |
| 17 | **Faith-Bound Form** | Holy, Warded | Safer near relics, shrines, or prayer; risk Ecclesiarchy attention. |
| 18 | **Xenos Psychic Geometry** | Forbidden, Xenos Tell | Achieve impossible targeting or perception; Ordo Xenos can identify trace. |
| 19 | **Heretek Psy-Circuit** | Forbidden, Heretek Signature | Interact with machines psychically; leave impossible logs. |
| 20 | **Black Archive Formula** | Forbidden, Unbound | Strong effect; roll a Psychic Burden. |

---

## 10. Psychic Burdens / Perils

Use this when a Psychic Power rolls a 1, when Corruption Fallout triggers, when a power is overchanneled, or when a Psychic Power is Powerful but Compromised.

| D20 | Burden | Trigger | Narrative Power | Mechanical Bite |
|---:|---|---|---|---|
| 1 | **Warp Scent** | Power is used under pressure | Something can track the psyker through the Immaterium. | Enemy Awareness +1 or Warp Trace clock. |
| 2 | **Soul Bleed** | Power affects a living mind | The psyker and target exchange emotional residue. | D3 Mind stress. |
| 3 | **Machine-Spirit Revulsion** | Power occurs near cogitators or sacred machines | Machines stutter, record, or resist. | Heat +1 if monitored; Mechanicus suspicion. |
| 4 | **Dream Infection** | Power touches fear, sleep, prophecy, or memory | Nearby sleepers dream the event. | Witness rumor or Public Panic clock. |
| 5 | **Possession Vector** | Power touches a vulnerable witness | A mind nearby becomes a door. | Create possession clock. |
| 6 | **False Revelation** | Power reveals hidden truth | The vision includes something true but misleading. | GM introduces a contaminated clue. |
| 7 | **Psyker’s Tell** | Power is visible or repeated | The psyker gains a recognizable mark. | D3 Shadow stress. |
| 8 | **Pain Feedback** | Power inflicts harm or force | Harm rebounds through the psyker’s body. | D3 Body stress. |
| 9 | **Patron Concern** | Power is public, extreme, or unauthorized | The Patron hears the cell is using witchcraft too freely. | Mark Patron Notice. |
| 10 | **Faith Panic** | Power is witnessed by faithful civilians | The act becomes miracle, witchcraft, or panic. | Heat +1 or Public Panic clock. |
| 11 | **Enemy Recognition** | Opposition has occult knowledge | The enemy recognizes the method. | Enemy Awareness +1. |
| 12 | **Psychic Fingerprint** | Power affects a target directly | The target carries the psyker’s trace. | Future tracking works both ways. |
| 13 | **Unclean Appetite** | Power succeeds cleanly | The power wants to be used again. | Radical Beat opportunity; D3 Corruption if refused at a key moment. |
| 14 | **Memory Fracture** | Power touches memory | Someone remembers what did not happen. | Create false witness or false clue. |
| 15 | **Warp Echo Object** | Power is channeled through gear | A nearby object becomes psychically charged. | Object gains Burden Profile. |
| 16 | **Thin Place** | Power is used at ritual site, shrine, death scene, or void breach | The location becomes easier to breach later. | Tick ritual/daemonic clock. |
| 17 | **Borrowed Voice** | Power involves speech, command, or message | Something else speaks with the psyker’s mouth. | D6 Mind or Corruption stress. |
| 18 | **Soul Debt** | Power prevents death or reveals impossible truth | A dead, daemonic, or forgotten intelligence claims payment. | Debt clock or Patron Notice. |
| 19 | **Open Door** | Power overreaches | The psyker opens a way in. | Possession pressure or Severe Corruption prompt. |
| 20 | **The Warp Answers** | Power solves a major problem | The answer is correct and catastrophic. | New Mission hook, Severe Fallout prompt, or major enemy move. |

### Possession Clock

The Possession clock is a 4-segment clock that tracks how close a character (or location) is to being fully taken over by a daemonic entity, warp intelligence, or psychic intruder. It advances when possession pressure enters play and the entity makes progress.

**When to tick:** +1 segment each time a psychic power with Possession Risk rolls a 1; each time a character suffers Severe or Critical Corruption Fallout in the presence of a daemonic entity; each time a possession attempt succeeds on a resistance check; each time the GM determines the entity has fed or grown stronger.

**Segment effects:**

| Segment | State | Effect |
|---|---|---|
| **1/4** | Whispers | The character hears a voice at the edge of perception. It knows things. It offers things. No mechanical effect yet, but the GM should roleplay the intrusion. The character takes D3 Corruption stress if they listen. |
| **2/4** | Blackouts | The character loses time. Minutes or hours vanish. They find themselves in places they don't remember going, with no account of how they got there. D6 Corruption stress. The GM may reveal the entity acted through them in small ways — a door opened, a name spoken, a weapon drawn. |
| **3/4** | Body Hijack | The entity can seize control of the character's body for brief moments — a single action, a few seconds. The character is aware but cannot resist. The GM controls the hijacked action. D6 Corruption + D6 Mind stress. The character's allies can see something is wrong. |
| **4/4** | Full Possession | The entity is in control. The character is still in there — somewhere — but they are not driving. The cell must decide: attempt an exorcism (Resist + Warp or Ecclesiarchy, Dangerous, with consequences for failure), bind the host (Seal the Vessel or similar ability), or end the host (kill the body to banish the entity). The Patron will have opinions. |

**Reducing the clock:** An exorcism (successful Resist + Warp or Ecclesiarchy roll at Dangerous or harder) reduces the clock by 1 segment on success, 2 on a 10. Sanctified wards, hexagrammic chains, or a Null Anchor ability can suppress advancement for one situation but do not reduce existing segments. If the host is killed, the entity is banished but not destroyed — it returns to the Warp and may seek a new anchor.

---

## 11. Example Psychic Powers

### Witch-Sight

```text
Discipline: Sight
Roll: Investigate + Warp
Effect: Perceive psychic residue, daemonic traces, emotional imprints, possession scars, or warp-thin places. Ask one direct question about immaterial influence. On a 10, ask two.
Tags: Subtle, Echoing
Cost: On 7 or lower, take D3 Corruption or Mind stress.
Tell: Eyes cloud, bleed, glow, or reflect impossible stars.
Trace: Soul Impression or Dream Echo.
Fallout Hook: You see something that sees you back.
```

### Soul Hook

```text
Discipline: Compulsion
Roll: Command + Warp
Effect: Seize a target’s attention through psychic pressure. They must answer one question, freeze, obey a short command, or reveal their strongest surface emotion.
Tags: Invasive, Predatory
Cost: On 7 or lower, take D6 Mind stress, or D6 Corruption if the target is tainted.
Tell: The air tightens. Nearby flames bend toward the psyker.
Trace: Psychic Fingerprint.
Fallout Hook: You leave a mark in the target, and the target leaves one in you.
```

### Telekinetic Rebuke

```text
Discipline: Force
Roll: Fight + Warp
Effect: Hurl, crush, pin, or disarm a target. Inflict D6 Body stress, or D8 if the environment is full of dangerous debris.
Tags: Violent, Visible
Cost: On 7 or lower, take D3 Body stress from backlash and D3 Corruption stress.
Tell: Blood beads at the nose, ears, or fingertips.
Trace: Machine Disruption or visible wreckage.
Fallout Hook: The force strikes something unintended.
```

### Pyroclastic Purge

```text
Discipline: Flame
Roll: Fight + Warp
Effect: Ignite a target, zone, infestation, corpse-pile, ritual circle, or swarm. Inflict D8 Body or Corruption stress against vulnerable targets.
Tags: Violent, Area, Visible, Miraculous
Cost: Always mark D3 Corruption stress after use. On 7 or lower, the fire spreads.
Tell: Flames burn in colors not found in nature.
Trace: Miracle Rumor or Warp Scent.
Fallout Hook: The fire consumes evidence, civilians, supplies, or something the cell needed.
```

### Mind-Scrape

```text
Discipline: Telepathy
Roll: Investigate + Warp
Effect: Tear a useful image, phrase, route, face, password, or emotional truth from a target’s mind.
Tags: Invasive, Forbidden
Cost: Take D3 Mind stress. On 7 or lower, also take D3 Corruption stress.
Tell: The target weeps, laughs, bleeds from the eyes, or forgets a personal memory.
Trace: Memory Scar.
Fallout Hook: You also pull out something implanted, alien, daemonic, or false.
```

### Veil of Unnotice

```text
Discipline: Veil
Roll: Skulk + Warp
Effect: Ordinary witnesses overlook you unless you draw direct attention. Cameras blur, memory softens, and casual observers rationalize your presence.
Tags: Subtle, Sustained
Cost: On 7 or lower, take D6 Shadow or Corruption stress.
Tell: People’s eyes slide away a moment too late.
Trace: Memory Scar or Machine Disruption.
Fallout Hook: Someone remembers you perfectly.
```

### Astropathic Needle

```text
Discipline: Message
Roll: Operate + Warp or Command + Warp
Effect: Send a short message through hostile psychic conditions without normal astropathic infrastructure.
Tags: Echoing, Invasive
Cost: Always take D3 Mind stress. On 7 or lower, take D6 Corruption stress.
Tell: Everyone nearby hears one word of the message in their own voice.
Trace: Dream Echo.
Fallout Hook: Something else answers.
```

### Biomantic Command

```text
Discipline: Flesh
Roll: Resist + Warp or Fight + Warp
Effect: Force a body to endure, heal, spasm, bleed, purge toxin, or rupture. Heal or reduce D6 Body stress, or inflict D6 Body stress.
Tags: Invasive, Visible
Cost: D3 Body or Corruption stress.
Tell: Veins darken, wounds steam, teeth crack, or old scars reopen.
Trace: Body remembers the touch.
Fallout Hook: The flesh obeys too literally.
```

### Hexagrammic Rebuke

```text
Discipline: Ward
Roll: Resist + Warp or Resist + Ecclesiarchy
Effect: Resist, reveal, delay, or repel a daemonic, psychic, or possession effect. Reduce incoming Corruption stress by one die size, or grant Protection 1 for one exchange.
Tags: Warded, Holy
Cost: If rushed, take D3 Mind or Authority stress. On 7 or lower, the ward burns out.
Tell: Chains snap taut, seals blacken, litanies repeat without breath.
Trace: Warded flare visible to psykers and daemons.
Fallout Hook: The ward holds by trapping part of the threat nearby.
```

### Machine Omen

```text
Discipline: Psy-Tech
Roll: Operate + Mechanicus or Investigate + Warp
Effect: Read psychic disturbance through machine behavior, auspex static, noospheric distortion, or servitor hesitation. Ask one question about a hidden psychic or occult influence.
Tags: Echoing, Heretek Risk
Cost: On 7 or lower, take D3 Corruption or Authority stress.
Tell: Machines chant error codes in dead voices.
Trace: Machine Disruption.
Fallout Hook: The machine-spirit reports you.
```

---

## 12. Forbidden Method Principles

Forbidden Methods are actions that cross a line.

They might be illegal, radical, heretical, cruel, politically catastrophic, or merely impossible to explain.

A Forbidden Method should usually:

- Work faster than the orthodox option.
- Grant permission, Mastery, a Resource, a clue, or a decisive effect.
- Create a trace, witness, debt, burden, or future pressure.
- Risk Authority, Shadow, Corruption, or Mind.
- Invite Patron Notice or rival judgment if exposed.

A Forbidden Method is not always supernatural.

Sometimes the forbidden thing is paperwork, torture, blackmail, jurisdictional theft, false confession, or sacrificing the innocent to preserve the mission.

If the method is certain, supported, and the only interesting question is cost, do not roll. Let it work and charge the listed cost.

If uncertainty matters, use the result bands below.

| Result | Forbidden Method Outcome |
|---|---|
| **10** | The benefit lands cleanly. Add one improvement: reduced trace, faster effect, extra clue, better cover, lower Heat, or less stress. |
| **8-9** | The benefit lands with the listed cost. |
| **6-7** | The benefit lands, but choose one meaningful cost: stress, Heat, Enemy Awareness, Patron Notice, Boon/Resource step-down, witness, debt, or clock. |
| **2-5** | Offer a lesser, delayed, or compromised benefit with a serious cost, or let the method work while creating major evidence. |
| **1** | The method still reveals something useful if it plausibly could, but trigger major exposure, Liability, Fallout prompt, catastrophic proof, or a hard enemy move. |

---

## 13. Forbidden Method Structure

Write a Forbidden Method as:

```text
Name:
Type:
Roll:
Benefit:
Tags:
Cost:
Exposure Risk:
Red Line:
Fallout Hook:
```

| Field | Meaning |
|---|---|
| **Name** | What the method is called. |
| **Type** | Authority, Interrogation, Daemonology, Xenos, Heretek, Psyker, Political, Purge, etc. |
| **Roll** | If uncertain or opposed. Some methods simply work and cost. |
| **Benefit** | What it gives the cell. |
| **Tags** | Traits that change consequences. |
| **Cost** | Immediate stress, Heat, Enemy Awareness, Burden, or Patron Notice. |
| **Exposure Risk** | What happens if discovered. |
| **Red Line** | What makes the Patron, Puritans, or rivals judge it. |
| **Fallout Hook** | How it creates story when stress becomes consequence. |

---

### Forbidden Assets

Forbidden Assets are Resource dice, not generic bonus pools.

| Asset Use | Effect |
|---|---|
| **Permission** | The asset makes an impossible action possible. |
| **Mastery** | The asset grants +1D10 when its narrow method applies. |
| **Leverage** | The asset changes faction, social, technical, occult, or investigative position. |
| **Temporary Protection** | The asset absorbs stress only when its effect says it can or the fiction clearly supports it. |
| **Decisive Effect** | The asset solves a logistical, occult, violent, or investigative barrier at a cost. |

A Forbidden Asset may step down when exposed, strained, fed, damaged, inspected, used on a 7 or lower, or used to solve a major problem.

Refresh a Forbidden Asset only when the fiction explains it: secrecy restored, repairs made, debt repaid, specialist help secured, a patron or faction appeased, a containment rite completed, the asset is fed, or the cell accepts a Burden or Liability.

---

## 14. Forbidden Method Tags

| Tag | Effect |
|---|---|
| **Atrocity** | If discovered, causes Authority stress, Heat, faction hostility, or Patron judgment. |
| **Blackmail** | Works through leverage; creates debt, retaliation, or future exposure. |
| **Coercive** | Gets answers or obedience but risks Mind, Authority, or unreliable truth. |
| **Compromising** | Evidence of use can be turned against the cell. |
| **Daemonological** | Risks Corruption, Warp Trace, possession, rival Inquisition, and Patron Notice. |
| **Disposable** | Burns an asset, witness, Cover, Contact, or Resource to solve a problem. |
| **False Flag** | Redirects blame; creates future enemy if discovered. |
| **Heretek** | Leaves machine evidence, Mechanicus hostility, scrapcode, or impossible logs. |
| **Jurisdictional Theft** | Seizes authority from another faction; causes formal complaint or rivalry. |
| **Purge** | Solves contamination through destruction; destroys evidence and creates public violence. |
| **Radical** | Counts toward Patron Notice or Red Line pressure if repeated or revealed. |
| **Scapegoat** | Reduces Heat by moving blame; creates guilt, revenge, or injustice. |
| **Secret-Keeper** | Requires witnesses to be silenced, recruited, mind-wiped, or compromised. |
| **Xenos** | Non-human logic or material creates Corruption, Mechanicus claim, or Ordo Xenos scrutiny. |
| **Warp Echo** | Leaves psychic evidence that can be tracked. |

---

## 15. Forbidden Method Chassis

Use a Chassis when building a radical shortcut.

| Chassis | Default Roll | Benefit | Default Tags | Default Cost |
|---|---|---|---|---|
| **Flash the Rosette** | Command/Procure/Investigate + Inquisition | Force compliance, access, jurisdiction, or emergency asset. | Jurisdictional Theft, Compromising | D6 Authority/Shadow; Heat +1/+2 |
| **Excruciator Protocol** | Command/Investigate + Inquisition/Heresy | Get the clue if the target knows it. | Coercive, Atrocity | D3 Mind/Authority; Shadow if exposed |
| **False Confession** | Deceive + Imperium/Heresy | Redirect suspicion or close an investigation. | Scapegoat, False Flag | Authority or Mind stress |
| **Controlled Purge** | Command/Fight + Imperium/Inquisition | Destroy contamination, witnesses, evidence, or threat. | Purge, Atrocity | Heat +2 or Authority stress |
| **Daemonological Consultation** | Investigate + Warp/Heresy | Ask one true question about daemon, rite, possession, or breach. | Daemonological, Warp Echo | Always Corruption stress |
| **Xenos Artefact Exploitation** | Operate/Investigate + Xenos | Do something Imperial tools cannot. | Xenos, Compromising | Corruption or Authority stress |
| **Heretek Bypass** | Operate + Mechanicus/Heresy | Open sealed systems, rewrite data, command machines. | Heretek, Traceable | Evidence clock or Authority stress |
| **Unsanctioned Psyker Asset** | Procure/Command + Warp/Underworld | Gain D8 Resource for prophecy, intrusion, or surveillance. | Secret-Keeper, Radical | Authority/Corruption stress |
| **Memory Work** | Investigate/Command + Warp/Inquisition | Remove, alter, or implant memory. | Coercive, Invasive, Secret-Keeper | Mind/Corruption stress |
| **Political Scapegoat** | Deceive/Command + Imperium | Reduce Heat, protect Cover, redirect faction anger. | Scapegoat, Blackmail | Authority/Mind stress |
| **Forbidden Archive Consultation** | Investigate + Inquisition/Heresy/Warp/Xenos | Gain clue, weakness, name, or method. | Radical, Compromising | Corruption/Authority stress |
| **Daemonhost Weapon** | Command + Warp/Inquisition | Gain D10 Resource against impossible or daemonic threats. | Daemonological, Atrocity | D6 Corruption; Patron Notice |
| **Xenos Containment Bargain** | Command/Investigate + Xenos | Use alien intelligence to solve alien threat. | Xenos, Blackmail | Enemy Awareness or Corruption |
| **Corpse Oracle** | Investigate + Warp/Heresy | Ask the dead one question. | Daemonological, Secret-Keeper | Corruption; witness risk |
| **Evidence Destruction** | Skulk/Operate + Imperium/Mechanicus | Remove proof, lower Heat, or protect Cover. | Compromising, Disposable | Lose clue or create audit clock |

---

## 16. Example Forbidden Methods

### Flash the Rosette

```text
Type: Authority
Benefit: Gain Mastery on Command, Procure, or Investigate; force compliance; seize jurisdiction; or turn failed Requisition into success at cost.
Tags: Jurisdictional Theft, Compromising
Cost: D6 Shadow or Authority stress.
Exposure Risk: Heat +1 if private; Heat +2 if public, recorded, or politically explosive.
Red Line: Using Patron authority for personal, Radical, or unauthorized ends.
Fallout Hook: Someone obeys while notifying a rival, superior, cult handler, or local power.
```

### Excruciator Protocol

```text
Type: Interrogation
Roll: Command + Inquisition, Investigate + Inquisition, or Command + Heresy
Benefit: You get the clue if the target knows it.
Tags: Coercive, Atrocity, Secret-Keeper
Cost: D3 Mind or Authority stress. If the target is innocent, also D6 Shadow stress.
Exposure Risk: Witnesses, medicae evidence, corpse disposal, confession reliability.
Red Line: Torturing the innocent, useful, protected, or politically visible.
Fallout Hook: The target says what you want to hear, implicates someone useful, dies, or reveals a truth you were not prepared for.
```

### Daemonological Consultation

```text
Type: Daemonology
Roll: Investigate + Warp or Investigate + Heresy
Benefit: Ask one true question about a daemon, ritual, possession, warp breach, or summoning.
Tags: Daemonological, Warp Echo, Radical
Cost: Always take D3 Corruption stress. On 7 or lower, take D6.
Exposure Risk: Any Puritan who learns of this may treat it as evidence against the cell.
Red Line: Bargaining, naming, feeding, preserving, or obeying the entity.
Fallout Hook: The answer includes a command, invitation, name, or price.
```

### Xenos Artefact Exploitation

```text
Type: Xenos
Roll: Operate + Xenos or Investigate + Xenos
Benefit: Achieve an effect Imperial tools cannot.
Tags: Xenos, Compromising, Forbidden
Cost: D3 Corruption or Authority stress. If used openly, also Shadow stress.
Exposure Risk: Possessing the device is incriminating by itself.
Red Line: Preserving, spreading, copying, bargaining with, or relying on alien logic.
Fallout Hook: The artefact teaches you how to use it better next time.
```

### Heretek Bypass

```text
Type: Heretek
Roll: Operate + Mechanicus or Operate + Heresy
Benefit: Open sealed systems, rewrite logs, command servitors, disable wards, bypass archive locks, or corrupt machine witnesses.
Tags: Heretek, Traceable, Compromising
Cost: D3 Authority or Corruption stress.
Exposure Risk: Leaves scrapcode, impossible logs, machine blasphemy, or Mechanicus claim.
Red Line: Creating persistent heretek systems, freeing abominable logic, or hiding evidence from the Patron.
Fallout Hook: The machine remembers you.
```

### Unsanctioned Psyker Asset

```text
Type: Psyker Asset
Benefit: Gain a D8 Resource for prophecy, surveillance, psychic intrusion, dream interpretation, or warp navigation.
Tags: Secret-Keeper, Radical, Compromising
Cost: D6 Authority or Corruption stress when first used.
Exposure Risk: Loyal authorities may demand execution, seizure, or Black Ship transfer.
Red Line: Protecting the asset over the mission or hiding it from the Patron.
Fallout Hook: The asset is right, but for the wrong reason.
```

### Memory Work

```text
Type: Psychic / Interrogation
Roll: Investigate + Warp or Command + Inquisition
Benefit: Remove, blur, implant, recover, or alter one memory.
Tags: Coercive, Invasive, Secret-Keeper
Cost: D3 Mind and D3 Corruption stress.
Exposure Risk: Memory scars can be detected by psykers, interrogators, and daemons.
Red Line: Altering allies, loyal servants, Bonds, or witnesses for convenience.
Fallout Hook: The false memory grows roots.
```

### Political Scapegoat

```text
Type: Political
Roll: Deceive + Imperium or Command + Imperium
Benefit: Reduce Heat by 1, protect a Cover, redirect faction anger, or close an inquiry.
Tags: Scapegoat, Blackmail, False Flag
Cost: D3 Authority or Mind stress.
Exposure Risk: Someone innocent or useful is endangered.
Red Line: Sacrificing a loyal servant of the Imperium for convenience.
Fallout Hook: The scapegoat survives, confesses falsely, or becomes a martyr.
```

### Controlled Purge

```text
Type: Purge
Roll: Command + Imperium, Fight + Militarum, or Command + Inquisition
Benefit: Destroy contamination, witnesses, hostile assets, evidence, or a compromised location.
Tags: Purge, Atrocity, Disposable
Cost: Heat +2 or D6 Authority stress.
Exposure Risk: Public violence, missing civilians, faction outrage, destroyed clues.
Red Line: Purging before proof, purging to hide the cell’s mistake, or sparing something worse.
Fallout Hook: Something survives inside the ashes.
```

### Forbidden Archive Consultation

```text
Type: Archive
Roll: Investigate + Inquisition, Heresy, Warp, or Xenos
Benefit: Gain a name, true history, ritual weakness, xenos pattern, daemon sign, or forbidden procedure.
Tags: Radical, Compromising
Cost: D3 Corruption or Authority stress.
Exposure Risk: Access logs, sealed citations, forbidden marginalia, Patron audit.
Red Line: Copying, teaching, preserving, or trading the knowledge.
Fallout Hook: The archive entry references the cell, Patron, or future event.
```

### Daemonhost Weapon

```text
Type: Daemonological Asset
Benefit: Gain a D10 Resource against daemonic, psychic, or impossible threats.
Tags: Daemonological, Atrocity, Hungry
Cost: Always take D6 Corruption stress. Mark Patron Notice when revealed.
Exposure Risk: Discovery may redirect the campaign.
Red Line: Allowing the daemonhost to remain useful after the immediate crisis.
Fallout Hook: The daemon does exactly what you asked, not what you meant.
```

---

## 17. Radical Use, Patron Notice, and Red Lines

Mark **Patron Notice** when a Psychic Power or Forbidden Method:

- Is used publicly.
- Uses Patron authority.
- Crosses a declared Red Line.
- Creates Heat 3, 6, or 9.
- Causes Severe Authority, Shadow, or Corruption Fallout.
- Preserves a forbidden asset after the mission.
- Hides evidence from the Patron.
- Gives a rival faction proof of Radical practice.
- Creates a new dependency.

### Escalating Radical Pattern

If the cell repeatedly uses the same forbidden type, create a clock.

| Clock | Segments | Advances When | When Full |
|---|---:|---|---|
| **Puritan Evidence File** | 6 | Forbidden method witnessed, logged, or reported | Rival Inquisition acts. |
| **Patron Doubt** | 4 | Patron Notice from Radical use | Patron demands proof, confession, or correction. |
| **Forbidden Dependency** | 6 | Method solves major problem | Cell needs the method again. |
| **Warp Claim** | 6 | Psychic/daemonological use under pressure | A Warp intelligence knows the cell. |
| **Mechanicus Hostility** | 4 | Heretek/Xenos machine evidence appears | Mechanicus claims jurisdiction. |
| **Witness of Atrocity** | 4 | Coercive, purge, or scapegoat method is seen | Witness becomes threat, martyr, or recruit. |

---

## 18. Containment, Refresh, and Recovery

Psychic and forbidden pressure should be recoverable, but never casually erased.

| Pressure | Refreshes or Clears When... |
|---|---|
| **Psychic tools** | Sanctioning rites, confession, containment, maintenance, reconsecration, specialist examination, or controlled rest. |
| **Wards and holy protections** | The ward is ritually repaired, reconsecrated, supplied with proper materials, confessed over, or reset in a sanctified place. |
| **Forbidden assets** | Secrecy is restored, repairs are made, the asset is fed, a debt is repaid, faction help is secured, or the cell accepts a Burden or Liability. |
| **Warp Trace** | Containment, misdirection, ritual cleansing, witness handling, machine-log correction, false trail, enemy distraction, or a stronger occult event obscures it. |
| **Radical suspicion** | The cell provides proof, scapegoat, confession, Patron report, rival evidence, or a costly act of loyalty. |

Do not clear Warp Trace just because the scene ends. Clear it when someone does the work or when a later consequence consumes it.

---

## 19. Calibrated Examples

| Example | Calibration |
|---|---|
| **Sanctioned Psyker uses Witch-Sight in a sealed shrine.** | Safe if prepared with focus and rites. On 7 or lower, take Corruption or Mind stress. If the trace matters, create Dream Echo or Soul Impression rather than Heat. |
| **Exorcist draws a circle while the possessed victim is breaking restraints.** | Risky or Dangerous depending on time pressure. Success grants Protection or delays possession. On 7 or lower, the ward burns out, marks the Exorcist, or ticks a possession clock. |
| **Untouchable enters a ritual chamber.** | Psychic powers nearby become Risky or Dangerous. Use Difficulty for the null interference, then Authority or Mind stress if the blank's nature becomes visible or socially intolerable. |
| **Radical Asset uses a xenos shard to open a sealed route.** | The shard grants permission and possibly Mastery. Cost should be Corruption, Enemy Awareness, Mechanicus claim, Resource step-down, or Patron Notice if exposed. |
| **Patron offers daemonological consultation.** | Let the answer work. Charge Corruption stress or Patron Notice, then decide whether the cost is Warp Trace, Red Line pressure, or a future demand. |
| **Psychic violence in a crowded hab-block.** | The attack may be Risky or Dangerous because of witnesses. Use Body/Corruption stress for backlash, Heat for panic, and Enemy Awareness if the cult recognizes the psyker's signature. |

---

## 20. Random Psychic / Forbidden Generator

Use this when a player asks for a strange power, a radical asset, a black archive tool, or an enemy method.

### Step 1: Roll Type

| D20 | Type |
|---:|---|
| 1-7 | Psychic Power |
| 8-13 | Forbidden Method |
| 14-16 | Forbidden Asset |
| 17-18 | Patron-sanctioned but morally dangerous tool |
| 19 | Enemy method the cell can steal |
| 20 | Powerful but Compromised story method |

### Step 2A: Psychic Power Chassis

| D20 | Chassis |
|---:|---|
| 1-2 | Witch-Sight |
| 3-4 | Soul Pressure |
| 5-6 | Telekinetic Force |
| 7-8 | Warp Fire |
| 9-10 | Mind-Scrape |
| 11-12 | Veil |
| 13-14 | Astropathic Needle |
| 15-16 | Biomantic Surge |
| 17-18 | Ward-Rite |
| 19-20 | Machine Omen |

### Step 2B: Forbidden Method Chassis

| D20 | Chassis |
|---:|---|
| 1 | Flash the Rosette |
| 2 | Excruciator Protocol |
| 3 | False Confession |
| 4 | Controlled Purge |
| 5 | Daemonological Consultation |
| 6 | Xenos Artefact Exploitation |
| 7 | Heretek Bypass |
| 8 | Unsanctioned Psyker Asset |
| 9 | Memory Work |
| 10 | Political Scapegoat |
| 11 | Forbidden Archive Consultation |
| 12 | Daemonhost Weapon |
| 13 | Xenos Containment Bargain |
| 14 | Corpse Oracle |
| 15 | Evidence Destruction |
| 16 | Blackmail Chain |
| 17 | Witness Disappearance |
| 18 | False Flag Attack |
| 19 | Soulguilt Scan |
| 20 | Black Archive Method |

### Step 3: Add a Module or Twist

| D20 | Module / Twist |
|---:|---|
| 1 | Subtle and hard to detect |
| 2 | Publicly impressive or miraculous |
| 3 | Requires blood, pain, or confession |
| 4 | Works only through a specific object |
| 5 | Works better on the guilty |
| 6 | Works better on the innocent |
| 7 | Leaves machine evidence |
| 8 | Leaves psychic evidence |
| 9 | A faction can identify it |
| 10 | The Patron secretly approves |
| 11 | The Patron publicly condemns it |
| 12 | It creates a useful clue |
| 13 | It destroys a useful clue |
| 14 | It protects a witness |
| 15 | It endangers a Bond or Contact |
| 16 | It has been used here before |
| 17 | It belongs to a rival cell |
| 18 | It resembles the enemy’s method |
| 19 | It works once cleanly, then hungers |
| 20 | It solves this problem and creates the next mission |

### Step 4: Add Cost

| D20 | Cost |
|---:|---|
| 1-3 | D3 Corruption stress |
| 4-5 | D6 Corruption stress |
| 6-7 | D3 Mind stress |
| 8-9 | D3 Shadow stress |
| 10-11 | D3 Authority stress |
| 12 | Heat +1 |
| 13 | Enemy Awareness +1 |
| 14 | Mark Patron Notice |
| 15 | Tick an Evidence Review clock |
| 16 | Tick a faction hostility clock |
| 17 | Create a witness |
| 18 | Create a debt |
| 19 | Add a Burden Profile |
| 20 | Create a new Mission hook |

---

## 21. One-Page Reference

### Psychic Power Quick Use

1. Name the power and desired effect.
2. Choose Skill + Domain.
3. Apply Difficulty based on preparation, exposure, danger, wards, opposition, null pressure, and impossible clean success.
4. Roll.
5. On 7 or lower, apply Corruption stress unless the power says otherwise.
6. Decide whether the power leaves Heat, Warp Trace, Enemy Awareness, witness pressure, or Patron Notice.

### Forbidden Method Quick Use

1. Name the shortcut.
2. Say why it works faster than the orthodox method.
3. Give the cell the benefit if the fiction supports it.
4. Roll only if uncertainty matters; otherwise let it work and charge the cost.
5. On 6-7, give the benefit with one meaningful cost. On 2-5, give a lesser, delayed, or compromised benefit with serious cost.
6. Decide what proof, trace, debt, Burden, clock, or Patron pressure remains.

### Fast Calibration

| Question | Use |
|---|---|
| Is the danger present before the roll? | Difficulty. |
| Does the acolyte personally pay? | Stress. |
| Did outsiders see or record it? | Heat. |
| Did the enemy learn a specific method or signature? | Enemy Awareness. |
| Did occult residue remain? | Warp Trace. |
| Would the Patron or rival Inquisition judge this? | Patron Notice, Patience, Red Line, or Authority stress. |
| Should the cost mature later? | Burden or clock. |

### Quick Costs

| Method | Usually Risks |
|---|---|
| Psychic sight | Mind, Corruption, Warp Trace |
| Psychic compulsion | Mind, Corruption, witness trauma |
| Psychic violence | Body, Corruption, Heat |
| Psychic concealment | Shadow, Corruption, strange memory |
| Daemonology | Corruption, Patron Notice, rival Inquisition |
| Xenos tools | Corruption, Authority, Mechanicus/Ordo claim |
| Heretek methods | Authority, Evidence Review, Mechanicus hostility |
| Torture/coercion | Mind, Authority, Shadow, unreliable truth |
| Purge | Heat, Authority, lost evidence |
| Scapegoat | Mind, Authority, future revenge |
| Rosette abuse | Shadow, Authority, Heat, Patron Notice |

### Best Question

When a player uses a Psychic Power or Forbidden Method, ask:

**What does this solve now, and what does it make worse later?**

---

## 22. Design Notes

Psychic Powers and Forbidden Methods should not replace investigation.

They should accelerate it, distort it, and make it dangerous.

The best use of a forbidden option is one that creates a real table argument:

- This will work.
- This will cost us.
- The orthodox way is slower.
- The Patron might approve.
- The Patron might kill us.
- The truth may not survive without it.
- We may not survive with it.

When in doubt, let the method work.

Then make the consequence specific.
